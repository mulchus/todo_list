...
async def show_tasks(
        event: CallbackQuery, butto: Button, dialog_manager: DialogManager,
        **kwargs,
):
    try:
        ...
        await dialog_manager.switch_to(SG.main_menu)
        return
...
dialog = Dialog(
    Window(
        Const("Главное меню"),
           Row(Button(Const("Показать текущие задачи"), id="show_tasks", on_click=show_tasks),
